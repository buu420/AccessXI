...
-- Builder revision 4: the interval was narrowed away from a lateral slide
-- hazard by AgentRadius + V2_HAZARD_MARGIN. Informational per portal; the
-- policy flag says the pass ran over every portal.
local PORTAL_HAZARD_MARGIN_TRIMMED = 0x10;
...
-- Tracking margin the builder's hazard pass uses, beyond the body radius,
-- when it narrows a certified interval away from a lateral slide hazard.
-- This pin means "hazard-margin PROCESSING was applied to every portal"; it
-- is NOT a per-portal guarantee of 1.5 yalms -- a doorway that cannot afford
-- the margin keeps a body width centred on what survived, deliberately less.
-- Pinned here because nothing in the file carries the number; the policy
-- flag only says the pass ran.
local V2_HAZARD_MARGIN = 1.5;
...
local function load_v2(file_size, storage, expected_zone, expected_source_crc,
    ...
    -- Revision 4 / flags 0x3: portal intervals carry the lateral slide-hazard
    -- margin (AgentRadius + V2_HAZARD_MARGIN from any face steeper than the
    -- climb grade beside the doorway). The beacon follows these intervals
    -- without any lateral margin of its own, so an artifact built without the
    -- trim is refused rather than walked along the lip of a bank.
    if (tonumber(policy.builder_revision) ~= 4) then
        return nil, 'unsupported AXWG v2 builder revision (need 4: hazard-margin trimmed portals)';
    end
    if (tonumber(policy.policy_flags) ~= 3) then
        return nil, 'unsupported AXWG v2 policy flags (need PORTAL_CAPACITY|HAZARD_MARGIN)';
    end
    ...
    for portal_id = 0, portal_count - 1 do
        ...
        if (bit.band(portal_flags, bit.bnot(0x1F)) ~= 0) then
    ...
    for edge_index = 0, edge_count - 1 do
        ...
        --
        -- The grade rule is enforced twice -- once by the builder when it emits
        -- an edge, and again here when the file is opened. Both judged a short
        -- rise over a short run as though it were a hillside, and that is
        -- exactly the shape of a staircase.
        --
        -- Live 2026-08-28, the ramp onto the La Theine Shattered Telepoint:
        -- run 0.333, rise 0.250, grade 0.750 = 36.9 degrees against a 33 degree
        -- limit. Every tread failed, in the CLIMBING direction only -- the
        -- descent passed the down-grade test -- so all three La Theine telepoint
        -- platforms became one-way islands the player could walk off and never
        -- onto. 594 nodes in that zone sit within one legal step-up of reachable
        -- ground with no edge to cross on.
        --
        -- The format already carries this idea: owner_step_ok below judges a
        -- portal's owner heights against max_step_up, not against a grade. This
        -- brings the edge rule into line with the portal rule and with the
        -- builder's own description of the constant -- "a kerb at a shared edge,
        -- not a ledge".
        --
        -- The hillside case is untouched. A long run with a big climb still
        -- fails, because its rise exceeds a step. Only a rise the policy already
        -- calls a legal step is exempt from also having to be a legal slope.
        if (upward_rise > 0
                and upward_rise > tonumber(policy.max_step_up) + 0.0001
                and (horizontal <= 0
                    or upward_rise
        ...
        if (upward_rise < 0
                and -upward_rise > tonumber(policy.max_step_down) + 0.0001
                and (horizontal <= 0
                    or -upward_rise
