...
function Test-AccessXiLegacyConfigTargetsRoot {
    param(
        [string]$ConfigPath,
        [string]$ReloadedRoot
    )
...
function Remove-LegacyAccessXiInstall {
    param(
    ...
    $sharedLegacyRoot = $unrelatedReloadedItems.Count -gt 0
    $ownsEntireReloadedRoot = $hasModMarker -and $hasAppMarker -and -not $sharedLegacyRoot
    ...
    $configRemoved = $false
    $fullLegacyRootRemoved = $false

    if ([string]::IsNullOrWhiteSpace($LegacyAccessXiConfigRoot)) {
        $LegacyAccessXiConfigRoot = Join-Path $env:APPDATA 'Reloaded-Mod-Loader-II'
    }
    $configRoot = Resolve-AccessXiCleanupPath $LegacyAccessXiConfigRoot
    ...
    if (-not $detected) {
        ...
        return [pscustomobject]([ordered]@{
            ...
            ConfigRemoved = $false
            FullLegacyRootRemoved = $false
            SharedLegacyRootPreserved = $false
        })
    }

    if (Test-Path -LiteralPath $configPath -PathType Leaf) {
        if ($ownsEntireReloadedRoot -and
            (Test-AccessXiLegacyConfigTargetsRoot -ConfigPath $configPath -ReloadedRoot $reloadedRoot)) {
    ...
    return [pscustomobject]([ordered]@{
        ...
        FullLegacyRootRemoved = [bool]$fullLegacyRootRemoved
        SharedLegacyRootPreserved = [bool]$sharedLegacyRoot
    })
}
