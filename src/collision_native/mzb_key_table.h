#pragma once

#include <array>
#include <cstdint>

namespace accessxi::collision {

// Exact FFXI MZB decode table extracted from the installed FFXiMain.dll
// image at file offset 0x4DEC0. It is format data, not imported parser code.
inline constexpr std::array<std::uint8_t, 256> mzb_key_table{
    ,
};

} // namespace accessxi::collision
