...
namespace
{
    ...
    void test_owner_registration_publishes_one_coherent_atomic_pair()
    {
        ...
        auto first = registration.snapshot();
        ...
        auto second = registration.snapshot();
        require(second.valid, "a replacement owner registration must be valid");
        require(second.owner == Label1, "the replacement owner pointer must survive");
        require(second.generation == 2, "a replacement owner must advance generation");

        registration.invalidate(Owner);
        auto wrong_invalidation = registration.snapshot();
        require(
            wrong_invalidation.valid &&
                wrong_invalidation.owner == Label1 &&
                wrong_invalidation.generation == 2,
            "destruction of an older owner must not invalidate the current owner");

        registration.invalidate(Label1);
        auto invalidated = registration.snapshot();
        require(!invalidated.valid, "destruction must atomically invalidate the current owner");
        require(invalidated.generation == 3, "destruction must advance owner generation");

        registration.publish(Owner);
        auto after_invalidation = registration.snapshot();
        require(
            after_invalidation.valid &&
                after_invalidation.owner == Owner &&
                after_invalidation.generation == 4,
            "publication after destruction must retain monotonic generation");
    ...
    void test_owner_registration_never_tears_pointer_from_generation()
    {
        ...
        {
            const auto snapshot = registration.snapshot();
            if (!snapshot.valid)
                continue;
            const uintptr_t expected =
                (snapshot.generation & 1u) != 0 ? Owner : Label1;
        ...
        require(final_snapshot.valid, "the final registration must be valid");
        require(final_snapshot.generation == 20000, "the final generation must not be lost");
        require(final_snapshot.owner == Label1, "the final owner must match its generation");
    }
...
int main()
{
    test_owner_registration_publishes_one_coherent_atomic_pair();
