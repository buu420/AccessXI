...
def render_transitions_lua(transitions: Sequence[Mapping[str, Any]]) -> str:
    ids = [str(row.get("transition_id", "")) for row in transitions]
    if any(not value for value in ids) or len(ids) != len(set(ids)):
        raise RouteEvidenceError("Transition IDs must be nonempty and unique.")
    return _lua_module("transitions", transitions, "transition_id")
...
def write_route_runtime_artifacts(
    *,
    ...
    transitions_bytes = render_transitions_lua(transition_rows).encode("utf-8")
    ...
    transition_registry_payload: bytes | None = None
    if contract_rows or transition_rows:
    ...
    transition_registry_payload = source_payloads.get("transition-registry")
