...
Assert-Contains $packageSource 'legacy_reloaded_cleanup\.ps1' 'Package builder must stage the ownership-bounded legacy cleanup library.'
...
Assert-Contains $installerSource '\$ReloadedConfigRoot' 'Installer must let cleanup tests isolate the old Reloaded config root.'
...
Assert-Contains $installerSource 'LegacyReloadedCleanup' 'Installer summary must record legacy cleanup decisions.'
...
if (Test-Path -LiteralPath $PackageRoot) {
    ...
    Assert-True (Test-Path -LiteralPath (Join-Path $PackageRoot 'legacy_reloaded_cleanup.ps1')) 'Package must contain the legacy Reloaded cleanup library.'
