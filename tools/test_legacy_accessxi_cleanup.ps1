...
$cleanupScript = Join-Path $RepoRoot 'installer\legacy_accessxi_cleanup.ps1'
if (-not (Test-Path -LiteralPath $cleanupScript -PathType Leaf)) {
    throw "Legacy Reloaded cleanup library is missing: $cleanupScript"
}
. $cleanupScript

if (-not (Get-Command Remove-LegacyAccessXiInstall -ErrorAction SilentlyContinue)) {
    throw 'Legacy Reloaded cleanup library must export Remove-LegacyAccessXiInstall.'
}
...
try {
    ...
    $ownedResult = Remove-LegacyAccessXiInstall `
        -InstallRoot $ownedInstallRoot `
        -PolExe $ownedPolExe `
        -LegacyAccessXiConfigRoot $ownedConfigRoot `
    ...
    $secondResult = Remove-LegacyAccessXiInstall `
        -InstallRoot $ownedInstallRoot `
        -PolExe $ownedPolExe `
        -LegacyAccessXiConfigRoot $ownedConfigRoot `
    ...
    $sharedLegacyRoot = New-OwnedReloadedRoot $sharedInstallRoot
    New-Item -ItemType Directory -Force -Path `
        (Join-Path $sharedLegacyRoot 'Apps\Ff7.En.Steam'), `
        (Join-Path $sharedLegacyRoot 'Mods\ff7.accessibility.reloaded') | Out-Null
    [System.IO.File]::WriteAllText((Join-Path $sharedLegacyRoot 'Apps\Ff7.En.Steam\AppConfig.json'), '{"AppId":"ff7_en.exe"}')
    [System.IO.File]::WriteAllText((Join-Path $sharedLegacyRoot 'Mods\ff7.accessibility.reloaded\ModConfig.json'), '{"ModId":"ff7.accessibility.reloaded"}')
    ...
    Write-TargetedReloadedConfig -ConfigRoot $sharedConfigRoot -ReloadedRoot $sharedLegacyRoot
    $sharedResult = Remove-LegacyAccessXiInstall `
        -InstallRoot $sharedInstallRoot `
        -PolExe (Join-Path $sharedPolRoot 'pol.exe') `
        -LegacyAccessXiConfigRoot $sharedConfigRoot `
    ...
    Assert-True (Test-Path -LiteralPath $sharedLegacyRoot) 'Cleanup removed a Reloaded root shared with Final Fantasy VII.'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $sharedLegacyRoot 'Mods\AccessXI.PolReloaded'))) 'Cleanup left the obsolete AccessXI mod in a shared Reloaded root.'
    Assert-True (-not (Test-Path -LiteralPath (Join-Path $sharedLegacyRoot 'Apps\AccessXI.PolPreLogin'))) 'Cleanup left the obsolete AccessXI app in a shared Reloaded root.'
    Assert-True (Test-Path -LiteralPath (Join-Path $sharedLegacyRoot 'Apps\Ff7.En.Steam\AppConfig.json')) 'Cleanup removed the Final Fantasy VII Reloaded app.'
    Assert-True (Test-Path -LiteralPath (Join-Path $sharedLegacyRoot 'Mods\ff7.accessibility.reloaded\ModConfig.json')) 'Cleanup removed the Final Fantasy VII accessibility mod.'
    ...
    Assert-True (-not $sharedResult.FullLegacyRootRemoved) 'Cleanup falsely reported removing a shared Reloaded root.'
    Assert-True $sharedResult.SharedLegacyRootPreserved 'Cleanup did not report preserving a Reloaded root shared with another game.'
    ...
    $partialResult = Remove-LegacyAccessXiInstall `
        -InstallRoot $partialInstallRoot `
        -PolExe (Join-Path $partialPolRoot 'pol.exe') `
        -LegacyAccessXiConfigRoot (Join-Path $testRoot 'partial-config') `
    ...
    $unrelatedResult = Remove-LegacyAccessXiInstall `
        -InstallRoot $unrelatedInstallRoot `
        -PolExe (Join-Path $unrelatedPolRoot 'pol.exe') `
        -LegacyAccessXiConfigRoot $unrelatedConfigRoot `
    ...
    $mismatchResult = Remove-LegacyAccessXiInstall `
        -InstallRoot $mismatchInstallRoot `
        -PolExe (Join-Path $mismatchPolRoot 'pol.exe') `
        -LegacyAccessXiConfigRoot $mismatchConfigRoot `
    ...
    $malformedResult = Remove-LegacyAccessXiInstall `
        -InstallRoot $malformedInstallRoot `
        -PolExe (Join-Path $malformedPolRoot 'pol.exe') `
        -LegacyAccessXiConfigRoot $malformedConfigRoot `
