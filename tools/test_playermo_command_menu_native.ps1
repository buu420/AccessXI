...
Assert-AddonPattern '(?s)function\s+accessxi\.playermo_command_menu_dat_entry.*?accessxi\.playermo_command_id_dat_entry\(command_id\)' 'self commands use the live native command ID'
Assert-AddonPattern '(?s)function\s+accessxi\.playermo_command_id_dat_entry.*?if\s*\(label\s*==\s*''''\s*and\s*help\s*==\s*''''\)\s*then' 'shared command mapper accepts help-only DAT rows'
