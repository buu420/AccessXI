...
if (-not (Test-Path -LiteralPath $sourcePath)) {
    throw "Missing native POL source: $sourcePath"
}
if (-not (Test-Path -LiteralPath $defPath)) {
    throw "Missing native POL exports file: $defPath"
}
...
Assert-Contains $exports 'AccessXI_POL_ReloadedInitialize=_AccessXI_POL_ReloadedInitialize@0' 'Native hook DLL must retain the Reloaded compatibility initializer.'
...
Assert-Contains $source 'dispatch_speech_sink_v1\s*\(\s*label\s*,\s*1\s*\)' 'Verified native labels must be dispatched as interrupting focus speech.'
Assert-Before $source 'dispatch_speech_sink_v1\s*\(\s*label\s*,\s*1\s*\)' 'append_reloaded_speech_queue\s*\(\s*label\s*\)' 'Direct in-process speech must be attempted before the legacy disk queue fallback.'

$v2 = [regex]::Match(
    $source,
    'AccessXI_POL_InitializeV2\s*\(\s*void\s*\)\s*\{(?<body>[\s\S]*?)\n\}\s*\n\s*extern\s+"C"[\s\S]*?AccessXI_POL_ReloadedInitialize',
    [System.Text.RegularExpressions.RegexOptions]::Singleline)
if (-not $v2.Success) {
    throw 'Native hook source must implement AccessXI_POL_InitializeV2.'
}
...
$compat = [regex]::Match(
    $source,
    'AccessXI_POL_ReloadedInitialize\s*\(\s*void\s*\)\s*\{(?<body>[\s\S]*?)\n\}\s*$',
    [System.Text.RegularExpressions.RegexOptions]::Singleline)
if (-not $compat.Success) {
    throw 'Native hook source must retain AccessXI_POL_ReloadedInitialize.'
}
Assert-Contains $compat.Groups['body'].Value 'g_reloaded_speech_queue_enabled\.store\s*\(\s*true\s*\)' 'Reloaded compatibility mode must retain its disk queue fallback.'
Assert-Contains $compat.Groups['body'].Value 'AccessXI_POL_InitializeV2\s*\(\s*\)' 'Reloaded compatibility initializer must delegate hook installation to V2.'

'ok: native PlayOnline hook speech ABI is versioned, direct, and fail-closed.'
