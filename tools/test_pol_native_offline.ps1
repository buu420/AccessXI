...
cmake --build $build --config $Configuration --target pol_native_queue_tests pol_postlogin_trace_tests pol_prelogin_semantics_tests pol_pml_selected_text_tests pol_pml_popup_text_tests pol_native_speech_worker_tests pol_native_host_tests
if ($LASTEXITCODE -ne 0) {
    throw "Offline native harness build failed with exit $LASTEXITCODE"
}
ctest --test-dir $build -C $Configuration -R 'pol_(native_(queue|speech_worker|host)|postlogin_trace|prelogin_semantics|pml_(selected|popup)_text)_tests' --output-on-failure
if ($LASTEXITCODE -ne 0) {
...
powershell -NoProfile -ExecutionPolicy Bypass -File (Join-Path $repo 'tools\test_pol_popup_notice_integration.ps1') -RepoRoot $repo
if ($LASTEXITCODE -ne 0) {
