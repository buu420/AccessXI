...
Assert-Contains $maskedPoll 'classify_pol_ui_control_role\s*\(' 'Masked-field polling must prove the visible control role.'
Assert-Contains $maskedPoll 'read_verified_masked_display_count\s*\(' 'Masked-field polling must use only the verified displayed mask count.'
Assert-Contains $maskedPoll 'masked_focus_speech\s*\(' 'Masked-field focus must report the accessible field state.'
